<?
	...
	class AtuadorBandaLarga extends Atuador {
		// Configuraчѕes de rede
		protected $net_cfg;
		
		
		// Arquivos de configuraчуo especэficos para cada tipo de atuador banda larga.
		protected $tcpip_cfg;		// Usado para fins gerais
		protected $pppoe_cfg;		// Usado apenas para o boot
		
		
		// Cache de informaчѕes (pra agilizar o processo)
		protected $ext_if;
		protected $nas_list;
		...
		/**
		 * Recebe uma instтncia do banco de dados
		 */
		public function __construct($bd=NULL,$debug=0) {
			...
			// Carrega arquivos de configuraчуo necessсrios
			...
			// Cacheia as informaчѕes relevantes
			$this->loadCacheInfo();
		...
		public function networkUP() {
			...
			while( list($iface,$dados) = each($n) ) {
				if( @$dados["status"] == "up" ) {
					...
					// Se nуo tiver ip e mсscara dс sѓ um "ifconfig $iface up"
		...
		// Retorna quais NAS estуo ativos neste sistema.
		...
		/**
		 * Os $parametros_pppoe sуo utilizados para que seja possэvel usar o mesmo atuador para
		 * Processar requisiчѕes PPPoE.
		 */
		...
		public function processa($op,$id_conta,$parametros,$parametros_pppoe="") {
			...
			// Este NAS nуo deveria estar chamando desta mсquina. Nуo estс habilitado ou configurado;
			if( !$int_if ) return;
			...
			switch($op) {
				...
				case 'a':
					 ...
					 *
					 * Parametros Adicionais PPPoE (situaчуo especial):
					 *	 interface,pid
					 *
					 */
					
					@list($rede,$mac,$up,$down,$user) = explode(",",$parametros);
					if( !$rede ) return;	// Se nуo recebeu a rede nуo faz nada.
					...
					if( $tipo_nas == "TCPIP" ) {
						...
						// A interface щ requerida para uso PPPoE
						if( !$iface ) return;
