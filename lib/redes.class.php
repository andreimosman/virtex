<?
   ...
   function to8bin($numero) {
      return( str_pad(d2b($numero),8,'0',STR_PAD_LEFT) );
   }
   ...
   function bin2addr($bin) {
      ...
      return(b2d($a).".".b2d($b).".".b2d($c).".".b2d($d));
   }

   function bitcount2bitmask($bitCount) {
   ...
   class RedeIP {
      ...
      function RedeIP($rede,$mask="") {
         ...
         $this->addr = addr2bin($rede);
         $this->network = $this->addr & $this->mask;
         $this->wildcard = str_pad( d2b( b2d($this->mask) ^ b2d(str_repeat('1',32))), 32, '0', STR_PAD_LEFT);
      ...
      function listaIPs() {
         $retorno = Array();
         for($x=b2d($this->network)+1;$x<b2d($this->broadcast);$x++) {
            $retorno[] = bin2addr(d2b($x));
            //echo bin2addr(d2b($x)) . "<br>\n";
         }
         return($retorno);
      }
      ...
      function minHost() {
         return( bin2addr(d2b(b2d($this->network)+1)) );
      }

      function maxHost() {
         return( bin2addr(d2b(b2d($this->broadcast)-1)) );
      }
      function numIPs() {
         return( b2d($this->broadcast) - b2d($this->network) + 1 );
      }
      ...
      function listaSubRedes($bits) {
         ...
         while( $rede < b2d($this->broadcast) ) {
            ...
            //echo "NW:".b2d($subrede->network) . "<br>NW:" . b2d($this->network) . "<br><br>\n";
            //echo "BC:".b2d($subrede->broadcast) . "<br>BC:" . b2d($this->broadcast) . "<br><br>\n";

            if( b2d($subrede->network) >= b2d($this->network) && b2d($subrede->broadcast) <= b2d($this->broadcast) ) {
      ...
      function imprimeDebug() {
         ...
         echo "A: " . $this->addr ."(". bin2addr($this->addr)  .")". "<br>\n";
         echo "M: " . $this->mask ."(". bin2addr($this->mask)  .")". "<br>\n";
         echo "N: " . $this->network ."(". bin2addr($this->network)  .")". "<br>\n";
         echo "W: " . $this->wildcard ."(". bin2addr($this->wildcard)  .")". "<br>\n";
         echo "BC:" . $this->broadcast ."(". bin2addr($this->broadcast)  .")". "<br>\n";
         echo "<hr>";
