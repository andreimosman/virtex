...
class VAPapocker extends VirtexAdmin {
	...
	public function processa($op="") {
		...
		if ($op == "acerto_contrato"){
			...
			$sSQL = "SELECT cp.id_cliente, cp.id_cliente_produto, cp.id_produto, cn.tipo_conta, p.valor FROM cbtb_cliente_produto cp, cntb_conta cn, prtb_produto p WHERE cp.excluido = false AND cp.id_cliente_produto = cn.id_cliente_produto AND cp.id_produto = p.id_produto";
			$produtos = $this->bd->obtemRegistros($sSQL);
			...
			for ($i=0;$i<count($produtos);$i++){
				...
				$sSQL  = "INSERT INTO cbtb_contrato (id_cliente_produto, id_produto, data_contratacao, vigencia, data_renovacao, valor_contrato, id_cobranca, status, tipo_produto, ";
				$sSQL .= "valor_produto, num_emails, quota_por_conta, tx_instalacao, valor_comodato, carencia) VALUES ( ";
				$sSQL .= "$id_cliente_produto, $id_produto, '2007-04-10',12,'2008-04-10', '$valor',$id_cobranca, 'A', '$tipo_produto', '$valor', '1', '50000', '0.00', '0.00', '20' ) ";
				$this->bd->consulta($sSQL);
				
				echo "SQL: $sSQL <br>";
				echo "NUM: $i<br>ICP: $id_cliente_produto<BR>IP: $id_produto<br>TIPO: $tipo_produto<br>VALOR: $valor<br><br><hr><br><br>";
