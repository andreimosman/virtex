	...
	function obtem($classe=null,$prop=null) {
		...
		if(!count($this->prefs[$classe])) {
			...
			if( $classe == "provedor" ) {
				...
				$sSQL = "SELECT ";
				$sSQL .= "tx_juros, multa, dia_venc, carencia, cod_banco, carteira, agencia, num_conta, convenio, observacoes, pagamento, path_contrato, cod_banco_boleto, carteira_boleto, agencia_boleto, conta_boleto, convenio_boleto, enviar_email, mensagem_email, email_remetente, cnpj_ag_cedente, codigo_cedente, operacao_cedente, div_agencia ";
				$sSQL .= "FROM pftb_preferencia_cobranca ";

			} else if ($classe == "total" ) {
				//Preferencias Totais
				$sSQL  = "SELECT ";
				$sSQL .= " pc.tx_juros, pc.multa, pc.dia_venc, pc.carencia, pc.cod_banco, pc.carteira, pc.agencia, pc.num_conta, pc.convenio, pc.observacoes, pc.pagamento, pc.path_contrato,cod_banco_boleto, carteira_boleto, agencia_boleto, conta_boleto, convenio_boleto, pc.enviar_email, pc.mensagem_email, pc.cnpj_ag_cedente, pc.codigo_cedente, pc.operacao_cedente, pc.div_agencia,  ";
